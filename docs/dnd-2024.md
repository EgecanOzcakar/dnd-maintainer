...
- **`WEAPON_CATALOG`** (hand-authored): `WeaponDef { id, category: 'simple' | 'martial', range: 'melee' | 'ranged', damageDice, damageType, properties[], weight, costGp, normalRange?, longRange?, versatileDice?, weaponProficiencyId, mastery? }`. Includes the 2024 firearms (`musket`, `pistol`).
...
- **`GEAR_CATALOG`** (hand-authored, bundled-quantity entries) + **`SRD_GEAR_CATALOG`** (generated, `src/lib/sources/srd-gear.ts`): full 2024 SRD adventuring gear, tools, foci, instruments. `ALL_GEAR_CATALOG` is the two concatenated — use it in UI.
- **`PACK_CATALOG`** (hand-authored) + **`SRD_PACK_CATALOG`** (generated): `ALL_PACK_CATALOG` is the union.
...
`npm run gen:items` (`scripts/gen-srd-items.ts`) reads vendored SRD snapshots in `scripts/data/`
and regenerates `src/lib/sources/srd-gear.ts`, `src/lib/sources/magic-items.ts`, and merges gear/pack
display names into `gamedata.json`. Deterministic — re-running produces no diff. Weapons/armor are
hand-tuned (proficiency ids, masteries); the script only prints a coverage diff for them. See
`scripts/data/README.md` for provenance and how to refresh the snapshots.

## Magic Items

Source-of-truth: `src/types/magic-items.ts` (`MagicItemDef`), `src/lib/sources/magic-items.ts`
(generated `MAGIC_ITEM_CATALOG`, ~262 entries).

- `MagicItemDef { id, name, category, rarity, attunement, attunementNote?, description, variantOf? }`.
- **Standalone reference data** — magic items are _not_ in the `ItemDef` union and have no inventory,
  attunement, or resolver wiring. Browsed at `/magic-items` (`src/pages/MagicItemsPage.tsx`).
- `name` and `description` are English SRD text rendered raw (not translated), same as character names.
  The CC-BY SRD 5.2.1 attribution (`SRD_ATTRIBUTION` in `src/lib/licenses.ts`) is shown on the page and
  embedded in the generated module header.
- Regenerate with `npm run gen:items`.
